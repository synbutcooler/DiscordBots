...
local OPCODES = {
    ...
    [29] = { name="GE",       fmt="rrr"  },
    [30] = { name="NOT",      fmt="rr"   },
    [31] = { name="PRINT",    fmt="rs"   },
    [32] = { name="JMP",      fmt="s"    },
    [33] = { name="JMPIF",    fmt="rs"   },
    [34] = { name="CLOSURE",  fmt="rr"   },
    [35] = { name="CALL",     fmt="rsr"  },
    [36] = { name="RETURN",   fmt="rs"   },
    [37] = { name="TAILCALL", fmt="rs"   },
    ...
    [53] = { name="GOTO",         fmt="u"    }, -- absolute PC (1-based). Sets PC = a-1 so normal advance lands on a.
    [54] = { name="S_SPREAD_INTO", fmt="rrc"  }, -- table-ctor vararg spread; c = positional count before append
}
local NUM_OPCODES = 55
...
local function compile(source)
        ...
        if not lex:check("op", ")") then
            repeat
                ...
                if lex:check("op", "...") then
                    lex:next()
                    -- `...` expands only in the LAST argument position;
                    -- elsewhere it is a single value (keep b=2 semantics).
                    if lex:check("op", ")") then
                    ...
                    local lst = cur().instructions[#cur().instructions]
                    if lst and lst.op == 35 and not lex:check("op", ",") then
                        -- bare call as the LAST argument: everything it returns
                        -- is forwarded (c=1 = multret in the VM).
                        lst.c = 1
                        splat = true
                        count = count + 1
                        break
        ...
        while not lex:check("op", "}") do
            ...
            elseif lex:check("id") and lex:peek2()
               and lex:peek2().type == "op" and lex:peek2().val == "=" then
                ...
                cur().reg_top = save + 1
            elseif lex:check("op", "...")
               and (lex:peek2() and lex:peek2().type == "op" and lex:peek2().val == "}") then
                -- `{..., x}` keeps single-value semantics; only a TRAILING `...`
                -- expands all varargs into the table.
                lex:next()
                local scratch = alloc_reg()
                emit(38, scratch, 1)
                emit(54, tbl_r, scratch, arr_idx)
                cur().reg_top = save
                break
            else
                arr_idx = arr_idx + 1
                local vrk = parse_expr(0)
                local cl = cur().instructions[#cur().instructions]
                if cl and cl.op == 35 and not lex:check("op", ",") then
                    -- LAST array element is a bare call: every result is
                    -- appended (multret), exactly like a trailing `...`.
                    cl.c = 1
                    emit(54, tbl_r, cl.a, arr_idx)
                    cur().reg_top = save + 1
    ...
    local function parse_suffixes(base_r)
        while true do
            ...
            elseif lex:check("op", ":") then
                ...
                local extra, m_splat = emit_args(self_slot + 1)
                if m_splat then
                    -- fixed args incl. self == extra (self + args parsed by
                    -- emit_args, which counts the trailing `...`/call slot, so
                    -- self is the offset that must be added back)
                    emit(35, call_base, -extra, 2)
                else
            ...
            elseif lex:check("op", "(") then
                ...
                local argc, c_splat = emit_args(call_base + 1)
                if c_splat then
                    -- fixed args == argc - 1 (the vararg slot is the last one)
                    emit(35, call_base, (argc == 1) and 0 or -(argc - 1), 2)
                else
    ...
    parse_expr = function(min_prec)
        ...
        local function primary()
            ...
            if t.type == "op" and t.val == "..." then
                lex:next(); local dst = alloc_reg(); emit(38, dst, 2); VARARG_REGS[Rrk(dst)] = true; return Rrk(dst)
            end
            ...
            if t.type == "op" and t.val == "(" then
                ...
                -- Parenthesised expressions are ALWAYS single-valued: if the
                -- inner expr ended in a bare CALL, park a self-MOVE after it so
                -- the multret detectors (args/return/table) don't fire on it.
                local pl = cur().instructions[#cur().instructions]
                if pl and pl.op == 35 then
                    local pr = to_reg(rk); emit(3, pr, pr)
                end
            ...
            if t.type == "id" and t.val == "KRS_ENCSTR" then
                ...
                lex:next()
                lex:expect("op", ")")
                local mr = to_reg(Krk(add_const(st.val)))
                return Rrk(parse_suffixes(mr))
            end
            ...
            if t.type == "id" and t.val == "KRS_ENCNUM" then
                ...
                local mr = to_reg(Krk(add_const({ __krs_encnum = true, v = num })))
                return Rrk(parse_suffixes(mr))
            end
        ...
        if t.type == "id" and t.val == "print" then
            ...
            repeat
                ...
                if lex:check("op", "...") then
                    ...
                    local pls = cur().instructions[#cur().instructions]
                    if pls and pls.op == 35 and not lex:check("op", ",") then
                        pls.c = 1
                        p_splat = true
                        count = count + 1
                        break
            ...
            lex:expect("op", ")")
            if p_splat then
                emit(31, base_r, (count == 1) and 0 or -(count - 1))
            else
            ...
            or lex:check("kw","until") or lex:peek() == nil then
                ...
                repeat
                    ...
                    if lex:check("op", "...") then
                        lex:next()
                        if lex:check("op", ",") then
                            -- `...` in a middle position is a single value
                            emit(38, dst, 2)
                        else
                            local save = cur().reg_top
                            cur().reg_top = dst + 1
                        ...
                        local rk  = parse_expr(0)
                        local rl  = cur().instructions[#cur().instructions]
                        if rl and rl.op == 35 and not lex:check("op", ",") then
                            -- `return x, f()` : trailing bare call forwards all
                            -- its results (c=1 = multret, consumed via VAREG).
                            rl.c = 1
                            r_splat = true
                            count = count + 1
                            break
                ...
                until not lex:match("op", ",")
                if r_splat then
                    emit(36, base_r, (count == 1) and 0 or -(count - 1))
                else
        ...
        if t.type == "kw" and t.val == "local" then
            ...
            if lex:match("op", "=") then
                if #names == 1 then
                    ...
                    local last = ins[#ins]
                    if VARARG_REGS[rhs_list[#rhs_list]] and #rhs_list <= #names then
                        -- a TRAILING `...` expands into the LHS slots. The
                        -- LHS regs are contiguous, so a fixed-count VARARG
                        -- (b = remaining+1) fills them and nil-pads shortages;
                        -- extra varargs are harmless (written past the regs).
                        for i = 1, #rhs_list - 1 do
                            local src = to_reg(rhs_list[i], dsts[i])
                            if src ~= dsts[i] then emit(3, dsts[i], src) end
                        end
                        emit(38, dsts[#rhs_list], #names - #rhs_list + 2)
                    elseif last and last.op == 35 and #rhs_list == 1 then
                        last.c = #names + 1
                        for i = 1, #names do
...
local CONTROL_FLOW = {
    ...
    RETURN = true, TAILCALL = true, HALT = true, GOTO = true,
}
...
local function transform_ins_list(list, constants)
    ...
    for old_pc, new_pc in pairs(old_to_new) do
        ...
        if ins then
            ...
            local function patch_abs(field)
                local old_abs = ins[field]
                if type(old_abs) ~= "number" then return end
                local new_target = old_to_new[old_abs]
                if not new_target then return end
                ins[field] = new_target
            end
...
-- Converts a linear instruction list with relative jumps into a
-- state-machine form:
--   1. Split into basic blocks
--   2. Assign each block a unique state id
--   3. Emit: LOADK state_reg, initial_state
--            dispatcher:
--              for each block:
--                if state == id then
--                  <block body with internal jumps rewritten as
--                   state = next_id; GOTO dispatcher>
--                end
--   Uses GOTO (absolute) + JMPIF for the dispatcher.
----------------------------------------------------------------------------
local STATE_REG = JUNK_REG_BASE + 50

local function flatten_cfg(list, constants)
    if not list or #list < 4 then return list end
    -- Only flatten with a reasonable probability so some functions stay linear
    if math.random() > 0.65 then return list end

    -- Skip lists that contain numeric-for opcodes; their relative offsets
    -- are tightly coupled to the linear layout and are hard to retarget
    -- correctly after block reordering.
    for _, ins in ipairs(list) do
        if ins.op == "FORPREP" or ins.op == "FORSTEP" then
            return list
        end
    end
    ...
    -- Helper: does this op terminate a basic block?
    local function is_terminator(op)
        return op == "JMP" or op == "JMPIF" or op == "RETURN"
            or op == "TAILCALL" or op == "HALT" or op == "GOTO"
            or op == "FORPREP" or op == "FORSTEP"
    end

    -- 1. Collect jump targets (1-based indices)
    local leaders = { [1] = true }
    for i, ins in ipairs(list) do
        if ins.op == "JMP" then
            local tgt = i + 1 + (ins.a or 0)
            if tgt >= 1 and tgt <= #list + 1 then leaders[tgt] = true end
            if i + 1 <= #list then leaders[i + 1] = true end
        elseif ins.op == "JMPIF" then
            local tgt = i + 1 + (ins.b or 0)
            if tgt >= 1 and tgt <= #list + 1 then leaders[tgt] = true end
            if i + 1 <= #list then leaders[i + 1] = true end
        elseif ins.op == "FORPREP" or ins.op == "FORSTEP" then
            local tgt = i + 1 + (ins.c or 0)
            if tgt >= 1 and tgt <= #list + 1 then leaders[tgt] = true end
            if i + 1 <= #list then leaders[i + 1] = true end
        elseif ins.op == "GOTO" then
            local tgt = ins.a or 0
            if tgt >= 1 and tgt <= #list + 1 then leaders[tgt] = true end
            if i + 1 <= #list then leaders[i + 1] = true end
        elseif is_terminator(ins.op) then
            if i + 1 <= #list then leaders[i + 1] = true end
        end
    end

    -- Build ordered list of leader PCs
    local leader_list = {}
    for pc in pairs(leaders) do
        if pc >= 1 and pc <= #list then
            leader_list[#leader_list + 1] = pc
        end
    end
    table.sort(leader_list)
    if #leader_list < 2 then return list end

    -- 2. Build basic blocks: { start, finish, state_id, body = {ins...}, successors }
    local blocks = {}
    local pc_to_block = {}
    for bi, start in ipairs(leader_list) do
        local finish = (leader_list[bi + 1] or (#list + 1)) - 1
        if finish < start then finish = start end
        local body = {}
        for j = start, finish do
            body[#body + 1] = {
                op = list[j].op, a = list[j].a, b = list[j].b,
                c = list[j].c, d = list[j].d, meta = list[j].meta or {}
            }
            pc_to_block[j] = bi
        end
        blocks[bi] = {
            start = start, finish = finish, state = bi,
            body = body, succ = {},
        }
    end
    ...
    for bi, blk in ipairs(blocks) do
        local last = blk.body[#blk.body]
        if not last then
            -- fallthrough
            if bi < #blocks then blk.succ[#blk.succ + 1] = bi + 1 end
        elseif last.op == "JMP" then
            local tgt = blk.finish + 1 + (last.a or 0)
            local tb = pc_to_block[tgt]
            if tb then blk.succ[#blk.succ + 1] = tb end
        elseif last.op == "JMPIF" then
            local tgt = blk.finish + 1 + (last.b or 0)
            local tb = pc_to_block[tgt]
            if tb then blk.succ[#blk.succ + 1] = tb end
            -- fallthrough (false branch)
            if bi < #blocks then blk.succ[#blk.succ + 1] = bi + 1 end
        elseif last.op == "GOTO" then
            local tb = pc_to_block[last.a or 0]
            if tb then blk.succ[#blk.succ + 1] = tb end
        elseif last.op == "RETURN" or last.op == "TAILCALL" or last.op == "HALT" then
            -- terminal
        elseif last.op == "FORPREP" or last.op == "FORSTEP" then
            local tgt = blk.finish + 1 + (last.c or 0)
            local tb = pc_to_block[tgt]
            if tb then blk.succ[#blk.succ + 1] = tb end
            if bi < #blocks then blk.succ[#blk.succ + 1] = bi + 1 end
        else
    ...
    -- Ensure we have a constant for each state id (and for 0)
    local function ensure_const(n)
        for i, c in pairs(constants) do
            if c == n then return i end
        end
        local idx = 0
        for k in pairs(constants) do
            if type(k) == "number" and k >= idx then idx = k + 1 end
        end
        constants[idx] = n
        return idx
    end
    ...
    local state_const = {}
    for bi = 1, #blocks do
        state_const[bi] = ensure_const(bi)
    end
    local zero_k = ensure_const(0)

    -- 3. Emit flattened code
    local out = {}
    local function emit(op, a, b, c, d)
        out[#out + 1] = make_ins(op, a, b, c, d)
    end

    -- Initial state load
    emit("LOADK", STATE_REG, state_const[1])

    -- Dispatcher entry point (absolute index will be patched later)
    local disp_pc = #out + 1

    -- For each block: JMPIF (state == id) then body else fall to next check
    -- We emit a chain of comparisons.
    local block_entry = {}  -- bi -> absolute index of its body start
    local after_checks = nil

    -- First emit all the state comparisons (dispatcher head)
    local jmp_to_body = {}  -- bi -> index of JMPIF that jumps to body
    for bi = 1, #blocks do
        -- tmp = (STATE_REG == state_const[bi]) via EQ
        local tmp = STATE_REG + 1
        emit("LOADK", tmp, state_const[bi])
        local eqr = STATE_REG + 2
        emit("EQ", eqr, STATE_REG * 2, tmp * 2)  -- rk encoding
        -- JMPIF not eq -> skip this body (relative, patched later)
        local ji = #out + 1
        emit("JMPIF", eqr, 0)  -- will patch b to skip over body
        jmp_to_body[bi] = ji
        block_entry[bi] = #out + 1  -- body starts here

        -- Emit body, rewriting terminators to state updates + GOTO disp
        local body = blocks[bi].body
        for ii, ins in ipairs(body) do
            local is_last = (ii == #body)
            if is_last and ins.op == "JMP" then
                local tgt = blocks[bi].finish + 1 + (ins.a or 0)
                local tb = pc_to_block[tgt]
                if tb then
                    emit("LOADK", STATE_REG, state_const[tb])
                    emit("GOTO", disp_pc)
                else
                    emit("GOTO", disp_pc)
                end
            elseif is_last and ins.op == "JMPIF" then
                -- VM semantics (see JMPIF codegen): `if (not REGS[a]) then
                -- PC = PC + b end` — JMPIF jumps when the register is FALSY,
                -- so its JUMP TARGET is the FALSE branch and the FALLTHROUGH
                -- is the TRUE branch.  The original rewrite had the two
                -- state assignments swapped, which inverted every flattened
                -- conditional (if/while/repeat/for-in picked the wrong arm).
                -- Now the fall path sets state = fallthrough (true) and the
                -- skip path sets state = jump target (false).
                local tgt = blocks[bi].finish + 1 + (ins.b or 0)
                local false_b = pc_to_block[tgt]
                local true_b = (bi < #blocks) and (bi + 1) or nil
                local skip = #out + 1
                emit("JMPIF", ins.a, 0)
                if true_b then
                    emit("LOADK", STATE_REG, state_const[true_b])
                    emit("GOTO", disp_pc)
                end
                -- JMPIF (cond false) lands right after the first GOTO
                out[skip].b = #out - skip
                if false_b then
                    emit("LOADK", STATE_REG, state_const[false_b])
                    emit("GOTO", disp_pc)
                end
elseif is_last and (ins.op == "RETURN" or ins.op == "TAILCALL" or ins.op == "HALT") then
                -- keep terminal as-is
                out[#out + 1] = {
                    op = ins.op, a = ins.a or 0, b = ins.b or 0,
                    c = ins.c or 0, d = ins.d or 0, meta = ins.meta or {}
                }
            elseif is_last and (ins.op == "FORPREP" or ins.op == "FORSTEP") then
                -- Keep the for opcode but retarget its relative jump to a
                -- state update + GOTO.  Simpler: emit original then force
                -- fallthrough state.
                out[#out + 1] = {
                    op = ins.op, a = ins.a or 0, b = ins.b or 0,
                    c = ins.c or 0, d = ins.d or 0, meta = ins.meta or {}
                }
                local fall = (bi < #blocks) and (bi + 1) or nil
                if fall then
                    emit("LOADK", STATE_REG, state_const[fall])
                end
                emit("GOTO", disp_pc)
            else
                -- ordinary instruction (or non-last terminator that we just copy)
                out[#out + 1] = {
                    op = ins.op, a = ins.a or 0, b = ins.b or 0,
                    c = ins.c or 0, d = ins.d or 0, meta = ins.meta or {}
                }
        ...
        -- After body we always GOTO dispatcher (already emitted for terminators).
        -- Patch the JMPIF that skips this body so it jumps past the body.
        local after_body = #out + 1
        local ji = jmp_to_body[bi]
        -- JMPIF: if *not* equal we want to skip body → relative offset
        out[ji].b = after_body - ji - 1
    end

    -- Final safety: if state is garbage, HALT
    emit("HALT")

    -- Fix absolute GOTO targets that point at disp_pc (they are already correct
    -- because we recorded disp_pc before emitting anything after the initial LOADK).
    -- But bodies may have been expanded, so absolute GOTOs that used the old
    -- disp_pc are still valid since disp_pc is the index of the first comparison.

    -- Re-patch any GOTO whose target is still the original disp_pc (already set).
    -- Also recompute absolute GOTOs that were emitted as disp_pc.
    for i, ins in ipairs(out) do
        if ins.op == "GOTO" and ins.a == disp_pc then
            ins.a = disp_pc  -- already correct
        end
    end
...
local function transform_pseudo(pseudo)
    ...
    pseudo.instructions = flatten_cfg(pseudo.instructions, consts)
    for i, pp in pairs(pseudo.protos) do
        pp.instructions = transform_ins_list(pp.instructions, consts)
        pp.instructions = flatten_cfg(pp.instructions, consts)
    end
    return pseudo
end
...
math.randomseed(0x5EED5EED + 12345) -- fixed: pre-driver draws must be reproducible across processes
...
function obf.obfuscate(source,seed)
    ...
    local fRETVALS=R()  local fVA=R()     local fVAREG=R()  local fVACNT=R()
    ...
    -- poison instead of hard error (optional – change if you want hard fail)
    ...
    do
        local vPROTO=R() local vENV2=R() local vCON2=R() local vPROTOS2=R() local vINNER=R() local vNVA=R()
        ...
        wl("  end")
        wl("  return function(...)")
        -- The exact vararg count (select('#',...)) must travel with the args:
        -- `{...}` keeps holes, so `#INNER` is a border and would silently drop
        -- trailing values when a nil sits inside the varargs.
        wl(("    local %s={...};local %s=select(\"#\",...)"):format(vINNER,vNVA))
        ...
        wl(("    return (%s[%s] or unpack)(%s(%s.%s,%s,%s,%s,%s,%s,%s,%s.%s or 0))"):format(
            _table,encodeString("unpack"),
            vEXEC,vPROTO,fINSTRUCTIONS,vCON2,vENV2,vPROTOS2,vINNER,vNVA,vUVLIST,vPROTO,fPARAMS))
        wl("  end")
        wl("end")
    end
    ...
    do
        local vT=R() local vI=R() local vFIX=R() local vCNT=R() local vVR=R()
        wl(("local function %s(%s)"):format(H[31],ha()))
        wl(("  local %s={};local %s=0b0"):format(vT,vFIX))
        wl(("  if(%s.b>0b0)then %s=%s.b elseif(%s.b<0b0)then %s=-%s.b end"):format(aINS,vFIX,aINS,aINS,vFIX,aINS))
        wl(("  for %s=0x0,%s-0b1 do %s=%s+0b1;%s[%s]=%s(%s[%s.a+%s])end"):format(vI,vFIX,vLIM,vLIM,vT,vLIM,_tostring,aREGS,aINS,vI))
        wl(("  if(%s.b<=0b0)then"):format(aINS))
        wl(("    local %s=%s.%s;local %s=%s.%s;%s.%s=0b0;%s.%s=0b0"):format(vCNT,aFRM,fVA,vVR,aFRM,fVAREG,aFRM,fVA,aFRM,fVAREG))
        wl(("    for %s=0b1,%s do %s=%s+0b1;%s[%s]=%s(%s[%s+%s-0b1])end"):format(vI,vCNT,vLIM,vLIM,vT,vLIM,_tostring,aREGS,vVR,vI))
        wl("  end")
        wl(("  print(%s[%s](%s,%s))end"):format(_table,encodeString("concat"),vT,encodeString("\t")))
    end
    wl(("local function %s(%s) %s.%s=%s.%s+%s.a;end"):format(H[32],ha(),aFRM,fPC,aFRM,fPC,aINS))
    -- GOTO: absolute jump. Set PC = a-1 so the normal advance step lands on a.
    wl(("local function %s(%s) %s.%s=%s.a-0b1;end"):format(H[53],ha(),aFRM,fPC,aINS))
    do
        local vCNT=R() local vI=R() local vLIM=R()
        wl(("local function %s(%s)"):format(H[54],ha()))
        local vLIM=R()
        wl(("  local %s=%s.%s;local %s=%s.c or 0b0;%s.%s=0b0"):format(vCNT,aFRM,fVA,vLIM,aINS,aFRM,fVA))
        -- c = number of positional elements already in the table; appending
        -- via `#t+1` stalls at a border when an earlier element is nil, so
        -- the array limit is carried in the operand and counted explicitly.
        wl(("  for %s=0b1,%s do %s=%s+0b1;%s[%s.a][%s]=%s[%s.b+%s-0b1];end"):format(vI,vCNT,vLIM,vLIM,aREGS,aINS,vLIM,aREGS,aINS,vI))
        wl("end")
    end
    ...
    do
        local vFN=R() local vARGS=R() local vRES=R() local vI=R() local vTN=R() local vJ=R() local vFIX=R() local vCNT=R()
        local vPACK=R() local vPK=R() local vVR=R() local vTN=R()
        wl(("local function %s(%s)"):format(H[35],ha()))
        wl(("  local %s=%s[%s.a]"):format(vFN,aREGS,aINS))
        wl(("  local %s={}"):format(vARGS))
        wl(("  local %s=0b0"):format(vFIX))
        wl(("  if(%s.b>0b0)then %s=%s.b-0b1 elseif(%s.b<0b0)then %s=-%s.b end"):format(aINS,vFIX,aINS,aINS,vFIX,aINS))
        wl(("  for %s=0b1,%s do %s[%s]=%s[%s.a+%s];end"):format(vI,vFIX,vARGS,vI,aREGS,aINS,vI))
        wl(("  local %s=%s"):format(vTN,vFIX))
        wl(("  if(%s.b<=0b0)then"):format(aINS))
        wl(("    local %s=%s.%s;local %s=%s.%s;%s.%s=0b0;%s.%s=0b0"):format(vCNT,aFRM,fVA,vI,aFRM,fVAREG,aFRM,fVA,aFRM,fVAREG))
        wl(("    for %s=0b1,%s do %s[%s+%s]=%s[%s+%s-0b1];end"):format(vJ,vCNT,vARGS,vFIX,vJ,aREGS,vI,vJ))
        wl(("    %s=%s+%s"):format(vTN,vTN,vCNT))
        wl("  end")
        wl(("  local %s=%s[%s] or function(...)local %s={...};%s.n=select(\"#\",...);return %s;end"):format(vPACK,_table,encodeString("pack"),vPK,vPK,vPK))
        wl(("  local %s=%s(%s((%s[%s] or unpack)(%s,0b1,%s)))"):format(vRES,vPACK,vFN,_table,encodeString("unpack"),vARGS,vTN))
        -- Exact bounds: `unpack(t)` stops at the first nil (#t is a border),
        -- which silently drops every argument after a nil.
        wl(("  if(%s.c==0b1)then"):format(aINS))
        wl(("    local %s=%s.n or #%s"):format(vCNT,vRES,vRES))
        wl(("    for %s=0b1,%s do %s[%s.a+%s-0b1]=%s[%s];end"):format(vJ,vCNT,aREGS,aINS,vJ,vRES,vJ))
        wl(("    %s.%s=%s;%s.%s=%s.a"):format(aFRM,fVA,vCNT,aFRM,fVAREG,aINS))
        wl(("  elseif(%s.c>0x01)then"):format(aINS))
        wl(("    for %s=0b1,%s.c-0x01 do %s[%s.a+%s-0x01]=%s[%s];end"):format(vJ,aINS,aREGS,aINS,vJ,vRES,vJ))
        wl("  end") wl("end")
    end
    do
        local vI=R() local vRES=R() local vFIX=R() local vCNT=R() local vVR=R() local vLIM=R()
        wl(("local function %s(%s)"):format(H[36],ha()))
        wl(("  local %s={}"):format(vRES))
        wl(("  local %s=0b0"):format(vLIM))
        -- `#RES+1` stalls at a border when a returned value is nil; count.
        wl(("  local %s=0b0"):format(vFIX))
        wl(("  if(%s.b>0b1)then %s=%s.b-0b1 elseif(%s.b<0b0)then %s=-%s.b end"):format(aINS,vFIX,aINS,aINS,vFIX,aINS))
        wl(("  for %s=0b0,%s-0b1 do %s=%s+0B1;%s[%s]=%s[%s.a+%s];end"):format(vI,vFIX,vLIM,vLIM,vRES,vLIM,aREGS,aINS,vI))
        wl(("  if(%s.b<=0b0)then"):format(aINS))
        wl(("    local %s=%s.%s;local %s=%s.%s;%s.%s=0b0;%s.%s=0b0"):format(vCNT,aFRM,fVA,vVR,aFRM,fVAREG,aFRM,fVA,aFRM,fVAREG))
        wl(("    for %s=0b1,%s do %s=%s+0b1;%s[%s]=%s[%s+%s-0b1];end"):format(vI,vCNT,vLIM,vLIM,vRES,vLIM,aREGS,vVR,vI))
        wl("  end")
        wl(("  %s.%s=true;%s.%s=%s"):format(aFRM,fSTOP,aFRM,fRETVALS,vRES))
        wl("end")
    end
    do
        local vFN=R() local vARGS=R() local vRES=R() local vI=R() local vTN=R()
        wl(("local function %s(%s)"):format(H[37],ha()))
        wl(("  local %s=%s[%s.a]"):format(vFN,aREGS,aINS))
        wl(("  local %s={}"):format(vARGS))
        wl(("  for %s=0B1,%s.b-0x01 do %s[%s]=%s[%s.a+%s];end"):format(vI,aINS,vARGS,vI,aREGS,aINS,vI))
        wl(("  local %s={%s((%s[%s] or unpack)(%s,0b1,%s))}"):format(vRES,vFN,_table,encodeString("unpack"),vARGS,vTN))
        wl(("  local %s=%s.b-0x01"):format(vTN,aINS))
        wl(("  %s.%s=true;%s.%s=%s"):format(aFRM,fSTOP,aFRM,fRETVALS,vRES))
        wl("end")
    end
    do
        ...
        wl(("    for %s=0b1,%s.%s do %s[%s.a+%s-0B1]=%s[%s];end"):format(vI,aFRM,fVACNT,aREGS,aINS,vI,vVA,vI))
        wl(("    %s.%s=%s.%s;%s.%s=%s.a"):format(aFRM,fVA,aFRM,fVACNT,aFRM,fVAREG,aINS))
        wl("  else")
    ...
    do
        ...
        local vPi=R() local vExtraVA=R() local vNV=R() local vAC=R()
        local vOPVAL=R() local vBID=R()
        wl(("%s=function(%s,%s,%s,%s,%s,%s,%s,%s)"):format(vEXEC,vCODE2,vCON2,vENV2,vPROTOS2,vVARARGS,vNV,vUPVALS,vNPARAMS))
        wl(("  local %s={}"):format(vREGS))
        wl(("  local %s=%s or {}"):format(vVARARGS, vVARARGS))
        wl(("  local %s=%s or 0b0"):format(vNPARAMS, vNPARAMS))
        wl(("  local %s=%s or 0b0;local %s=%s-%s;if(%s<0b0)then %s=0b0;end"):format(vNV, vNV, vAC, vNV, vNPARAMS, vAC, vAC))
        wl(("  for %s=0b1,%s do %s[%s-0b1]=%s[%s];end"):format(vPi,vNPARAMS,vREGS,vPi,vVARARGS,vPi))
        wl(("  local %s={}"):format(vExtraVA))
        -- Exact vararg count (not #VARARGS): `{...}` keeps nil holes, so the
        -- length operator would truncate the extra-args array at the border.
        wl(("  for %s=0b1,%s do %s[#%s+0b1]=%s[%s];end"):format(vPi,vAC,vExtraVA,vExtraVA,vVARARGS,vPi))
        wl(("  local %s={%s=0b1,%s=false,%s=%s,%s=%s or {},%s=0b0,%s=0b0,%s=%s}"):format(vFRM,fPC,fSTOP,fVARARGS,vExtraVA,fUPVALS,vUPVALS,fVA,fVAREG,fVACNT,vAC))
        ...
        local cf_ops = {32,33,36,37,39,40,41,53}
        local cf_perm = {}
        for _,o in ipairs(cf_ops) do cf_perm[#cf_perm+1] = op_perm[o] end
        wl(("  local %s={[%s]=true,[%s]=true,[%s]=true,[%s]=true,[%s]=true,[%s]=true,[%s]=true,[%s]=true}"):format(
            vCFCHECK, cf_perm[1], cf_perm[2], cf_perm[3], cf_perm[4], cf_perm[5], cf_perm[6], cf_perm[7], cf_perm[8]))
    ...
    do
        ...
        dynamic()
        wl(("%s(%s,%s,%s,%s,{},0b0,{},0b0)"):format(vEXEC, vCODE, vCONSTS, vENV, vPROTOS))
    end
    return table.concat(out,"\n")
end
...
cr.Parent=workspace['This script is protected with Kryos v16.2, by feariosz0']
end)
if(ok)then
x=nil
end
...
src="pcall(function()\n"..smth.."\nend)\n"..src
-- hotfix (v16.2-junk-safe): feariosz0's junk block can crash any
-- script that runs in an environment where Instance.new succeeds
-- (x=nil then x=x+d -> arithmetic on nil). Wrapping it in pcall
-- keeps the junk/watermark but makes it harmless.
